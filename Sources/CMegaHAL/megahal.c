...
{
    static char *newfilename=NULL;

    if(newfilename==NULL) newfilename=(char *)malloc(sizeof(char)*1);

    newfilename=(char *)realloc(newfilename,sizeof(char)*(strlen(directory)+strlen(SEP)+strlen(filename)+1));
    if(newfilename==NULL) error("initialize_error","Unable to allocate filename");

    sprintf(newfilename, "%s%s%s", directory, SEP, filename);
        
    if(errorfp!=stderr) fclose(errorfp);

    if(filename==NULL) return(TRUE);

    errorfp = fopen(newfilename, "a");
    if(errorfp==NULL) {
	errorfp=stderr;
	return(FALSE);
    }
...
bool initialize_status(char *filename)
{
    ...
    newfilename=(char *)realloc(newfilename,sizeof(char)*(strlen(directory)+strlen(SEP)+strlen(filename)+1));
    if(newfilename==NULL) error("initialize_status","Unable to allocate filename");

    sprintf(newfilename, "%s%s%s", directory, SEP, filename);
        
    if(statusfp!=stdout) fclose(statusfp);
    if(filename==NULL) return(FALSE);
    statusfp=fopen(newfilename, "a");
    if(statusfp==NULL) {
	statusfp=stdout;
	return(FALSE);
    }
...
void show_dictionary(DICTIONARY *dictionary)
{
    ...
    newfilename=(char *)realloc(newfilename,sizeof(char)*(strlen(directory)+strlen(SEP)+strlen("megahal.dic")+1));
    if(newfilename==NULL) error("show_dictionary","Unable to allocate filename");

    sprintf(newfilename, "%s%s%s", directory, SEP, "megahal.dic");
...
void set_default_directory(char *dir)
{
    if (defaultdirectory==NULL)
        defaultdirectory=(char *)malloc(sizeof(char)+1);
    
    defaultdirectory=(char *)realloc(defaultdirectory,strlen(dir)+1);

    if (defaultdirectory==NULL)
        error("set_default_directory","Unable to allocate filename");
    else {
        strcpy(defaultdirectory,dir);

        if (directory==NULL)
            directory=(char *)malloc(sizeof(char)+1);

        directory=(char *)realloc(directory,strlen(defaultdirectory)+1);
...
void load_personality(MODEL **model)
{
    ...
    if(strcmp(directory, defaultdirectory)!=0) {
	sprintf(filename, "%s%smegahal.brn", directory, SEP);
	file=fopen(filename, "r");
	if(file==NULL) {
...
void change_personality(DICTIONARY *command, int position, MODEL **model)
{
    ...
    if(directory == NULL) {
	directory = (char *)malloc(sizeof(char)*(strlen(defaultdirectory)+1));
	if(directory == NULL) {
