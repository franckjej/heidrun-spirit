...
# git + ca-certificates let SPM clone the heidrun-protocol dep over
# HTTPS. No libsqlite3-dev — unlike heidrun-server the bot has no
...
# Single RUN: build + stage. heidrun-protocol is fetched anonymously
# over HTTPS — the repo is public.
#
...
# bundle, which SwiftPM places next to the executable — so it must
# travel with the binary. The bundle's name is NOT hardcoded: its form
# is platform-specific (macOS: <pkg>_<target>.bundle; Linux:
# <pkg>_<target>.resources), so we `find` both. The non-Darwin
# Bundle.module accessor looks for the `.resources` directory next to
# the executable, i.e. /usr/local/bin — which is where /out's contents
# land.
